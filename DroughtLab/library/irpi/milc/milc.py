...
def MILC(name,data_input,PAR,fig,Wobs=[],K=0):
    
    FPAR=np.loadtxt('fixedpar')
    PIO=data_input['P']
    TEMPER=data_input['T']
    Qobs=data_input['Q']
    N=len(Qobs)
    ...
    lambd    = PAR[7] # initial abstraction coefficient
    Sr_coeff  = PAR[8] # multiplicative coefficient for Sr
    
    Ab      = FPAR[0];     # Basin area
    delta_T = FPAR[1];     # input data time step in hour
    ...
    for t in range(N):
        if (P[t]>0 and PIOprec==0):
        
            S=Sr_coeff*(W_max-W);
            Ia=lambd*S
        
        elif (P[t]==0 and PIOprec==0 and not(np.isnan(S))):
            S=np.nan
            Pcum=0
        ...
        if not(np.isnan(S)):
            if Pcum<Ia:
                ...
                Pcum1=Pcum+P[t]
            if Pcum>=Ia:
                 IE=((((Pcum1-Ia)**2)/(Pcum1+(1-lambd)*S))-(((Pcum-Ia)**2)/(Pcum+(1-lambd)*S)))
                 Pcum=Pcum1
        
        E=EPOT[t]*W/W_max
        PERC[t]=Nu*Ks*(W/W_max)**(m2)
        BF[t]=(1-Nu)*Ks*(W/W_max)**(m2)
        W=W+(P[t]-BF[t]-IE-PERC[t]-E)
...
if __name__ == '__main__':
   ...
   QobsQsim,data=MILC(name,data_input,PAR,fig,ASC,K=0.1)
